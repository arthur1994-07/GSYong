...
DxEffSingle* BLURSYS_PROPERTY::NEWOBJ ( boost::shared_ptr<DxEffSingleGroup>& spThisGroup )
{
	...
	//
	// 추가적으로 설명하면, 만약 DxEffSingle 에 있는 m_textureRes가 로딩 완료되면 m_spCompletedData 데이터는
	// EFF_PROPERTY 에 있는 m_textureRes 의 m_spCompletedData 과는 다른 것이 된다.
	//
	// 결론은 로딩이 안된 상태의 m_textureRes 를 복사한다는 건 문제가 될 수도 있다.
	//
