...
BOOL CStringMemory::Open(const std::string& ZipFileName, LPCSTR _szFileName, BOOL bDecode, BOOL bPack)
{
	MIN_ASSERT(m_pBufferMem==NULL&&"CStringMemory::Open ()");

	if( bPack )
	{
		CString strPath(ZipFileName.c_str());
		strPath += _szFileName;

		SFileHandle * pFileHandle = sc::file::g_sFileSystem.OpenFile( strPath );
		if( pFileHandle )
		{
	...
	{
		//CUnzipper cUnzip;
		//m_nBufSize = cUnzip.UnzipToMemory(ZipFileName, _szFileName, m_pBufferMem);
        m_nBufSize = ZipMan::GetInstance().UnzipToMemory(ZipFileName, _szFileName, m_pBufferMem);        
	}
	...
	if( m_bDecode )
	{
        ...
        if( sc::CRijndael::VERSION == nVersion )
		{
			m_nVersion = nVersion;
			...
			m_pRijndael->DecryptEx((char*) m_pBufferMem, m_nBufSize);
		}
        else
        {
			return FALSE;
