...
{
    m_textureResMove = TextureManager::GetInstance().LoadTexture(
        m_szTexMove,
        false,
    ...
    m_textureResRotate = TextureManager::GetInstance().LoadTexture(
        m_szTexRotate,
        false,
        TEXTURE_ADD_DATA::TEXUTRE_TYPE_2D,
        0,
...
{
	m_textureResMove.Clear();
    m_textureResRotate.Clear();
...
HRESULT DxEffectWaterLight::Render ( DxFrame *pframeCur, LPDIRECT3DDEVICEQ pd3dDevice, DxFrameMesh *pFrameMesh, DxLandMan* pLandMan )
{
	...
	if ( m_pVB )
	{
		...
		pd3dDevice->SetTexture( 0, m_textureResMove.GetTexture() );
		pd3dDevice->SetTexture( 1, m_textureResRotate.GetTexture() );
...
HRESULT DxEffectWaterLight::CloneWaterLight ( LPDIRECT3DDEVICEQ pd3dDevice, DxMeshes *pmsMeshs )
{
	...
	// 처음 디폴트 값을 넣어준다.
	m_fHighHeight = m_fLowerHeight = pSrcVertices[0].vPos.y;

	// 최고점과 최저점을 찾는다.
	for ( DWORD i=0; i<m_dwVertices; i++ )
	{
		if ( m_fHighHeight < pSrcVertices[i].vPos.y )
			m_fHighHeight = pSrcVertices[i].vPos.y;
		if ( m_fLowerHeight > pSrcVertices[i].vPos.y )
			m_fLowerHeight = pSrcVertices[i].vPos.y;
	}
	...
	{
		float fTempAlpha;
		if ( pSrcVertices->vPos.y-m_fMiddleHeight >= 0.f )
			fTempAlpha = ( (m_fHighHeight-pSrcVertices->vPos.y) / (fDis/2.f)) * (m_iAlphaMiddle - m_iAlphaUp) + m_iAlphaUp;
		else
			fTempAlpha = ( (m_fLowerHeight-pSrcVertices->vPos.y) / (fDis/2.f)) * (m_iAlphaMiddle - m_iAlphaDown) + m_iAlphaDown;

		fTempAlpha = fabsf(fTempAlpha);

		pDestVertices->Pos = pSrcVertices->vPos;
		pDestVertices->Diff = D3DCOLOR_ARGB ( (int)fTempAlpha, (int)m_vColor.x, (int)m_vColor.y, (int)m_vColor.z );
		pDestVertices->Tex1 = D3DXVECTOR2((pSrcVertices->vPos.x+pSrcVertices->vPos.y)*0.01f/m_fScale,(pSrcVertices->vPos.z+pSrcVertices->vPos.y)*0.01f)/m_fScale;	// 스케일
...
void DxEffectWaterLight::SaveBuffer ( sc::SerialFile &SFile )
{
	//	읽지 않고 스킵용으로 사용됨.
...
void DxEffectWaterLight::LoadBufferSet ( sc::SerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	DWORD dwBuffSize;
	WATERVERTEX* pTemp;
	WORD*		 pIndices;
...
void DxEffectWaterLight::LoadBuffer ( sc::SerialFile &SFile, DWORD dwVer, LPDIRECT3DDEVICEQ pd3dDevice )
{
	DWORD dwBuffSize;
	PBYTE pbBuff;
	PBYTE pbPoints;
...
HRESULT DxEffectWaterLight::CloneData ( DxEffectBase* pSrcEffect, LPDIRECT3DDEVICEQ pd3dDevice, DxTextureEffMan* pTexEff )
{
	HRESULT hr;
