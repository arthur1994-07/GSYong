...
const DWORD DxEffectSpore::FLAG = _EFF_SINGLE_AFTER_1;
...
sc::ObjectPool<DxEffectSpore::PARTICLE> *DxEffectSpore::m_pParticlePool	= NULL;
sc::ObjectPool<DxEffectSpore::FIREFLY> *DxEffectSpore::m_pFireFlyPool		= NULL;
...
void DxEffectSpore::CleanUp_Particle()
{
    //if ( !m_pParticlePool )
    //{
    //    m_pParticleHead = NULL;
...
void DxEffectSpore::CleanUp_FireFly()
{
    //if ( !m_pFireFlyPool )
    //{
    //    m_pFireFlyHead = NULL;
	...
	FIREFLY* pDel = NULL;
	FIREFLY* pCur = m_pFireFlyHead;
	while( pCur )
	{
		pDel = pCur;
		pCur = pCur->pNext;

        //m_pFireFlyPool->releaseMem( pDel );
        SAFE_DELETE( pDel );
	}
...
void DxEffectSpore::OnCreateDevice_STATIC ( LPDIRECT3DDEVICEQ pd3dDevice )
{
    //m_pParticlePool = new sc::ObjectPool<PARTICLE>;
    //m_pFireFlyPool = new sc::ObjectPool<FIREFLY>;
}

void DxEffectSpore::OnDestroyDevice_STATIC ()
{
	//SAFE_DELETE(m_pParticlePool);
	//SAFE_DELETE(m_pFireFlyPool);
}

void DxEffectSpore::OnResetDevice_STATIC ( LPDIRECT3DDEVICEQ pd3dDevice )
{
    D3DCAPSQ d3dCaps;
...
void DxEffectSpore::OnLostDevice_STATIC ( LPDIRECT3DDEVICEQ pd3dDevice )
{
...
//
//			pRelease->pNext = NULL;
//			m_pParticlePool->releaseMem(pRelease);
//		}
//	}
//}
...
void DxEffectSpore::PutSleepParticle ( PARTICLE* pParticle )
{
 ...
 //       m_pParticlePool->releaseMem(pParticle);
	//}
...
void DxEffectSpore::PutSleepFireFly ( FIREFLY* pFireFly )
{
	...
	pFireFly->pNext = NULL;
    SAFE_DELETE( pFireFly );
}

HRESULT DxEffectSpore::InitDeviceObjects ( LPDIRECT3DDEVICEQ pd3dDevice )
{
    m_textureResSpore = TextureManager::GetInstance().LoadTexture(
        m_szSporeTex,
        false,
    ...
    m_textureResFireFly = TextureManager::GetInstance().LoadTexture(
        m_szFireFlyTex,
        false,
        TEXTURE_ADD_DATA::TEXUTRE_TYPE_2D,
        0,
...
{
	m_textureResSpore.Clear();
    m_textureResFireFly.Clear();
...
HRESULT DxEffectSpore::RenderSpore ( LPDIRECT3DDEVICEQ pd3dDevice )
{
...

	{
		...
		pd3dDevice->SetTexture( 0, m_textureResSpore.GetTexture() );
		...
		if ( m_bRotate )	// 회전 한다.
		{
			...
			while ( pParticle )
			{
				DxBillboardLookAt ( matLocal, &pParticle->vNowPos, DxViewPort::GetInstance().GetMatView() );
...
HRESULT DxEffectSpore::CreateParticle ()
{
	...
	{
        //PARTICLE* pParticle = m_pParticlePool->getMem();
...
HRESULT DxEffectSpore::CreateFireFly ()
{
	...
	fRandHeight = (sc::Random::RANDOM_POS()*25.f) - 5.f;		//	-5.f ~ 15.f
	vPos		= D3DXVECTOR3((float)((rand()%400)-200)+vLookatPt.x, vLookatPt.y+fRandHeight, (float)((rand()%400)-200)+vLookatPt.z);
	vDirect		= D3DXVECTOR3(sc::Random::RANDOM_POS()-0.5f, sc::Random::RANDOM_POS()-0.5f, sc::Random::RANDOM_POS()-0.5f );
	...
	if ( vPos.x < m_vMax.x && vPos.x > m_vMin.x && vPos.z < m_vMax.z && vPos.z > m_vMin.z )
	{
		//FIREFLY* pParticle = m_pFireFlyPool->getMem();
        FIREFLY* pParticle = new FIREFLY;
		...
		pParticle->fAplhaTime	= 2.f + sc::Random::RANDOM_POS() * 5.f ;
...
HRESULT DxEffectSpore::UpdateFireFly ( LPDIRECT3DDEVICEQ pd3dDevice )
{
	...
	while ( pParticle )
	{
		...
		if ( pParticle->vNowPos.y < (vLookatPt.y-7.f) )			// 낮으면 위로 올리고
		{
			...
			vDirect		+= D3DXVECTOR3(sc::Random::RANDOM_POS()-0.5f, sc::Random::RANDOM_POS(), sc::Random::RANDOM_POS()-0.5f);
			D3DXVec3Normalize ( &vDirect, &vDirect );
			pParticle->vDirect		= vDirect;
			pParticle->fSpeed		= 1.f + sc::Random::RANDOM_POS();			
		}
		...
		if ( pParticle->fChange > 1.5f )						// 가끔씩은 움직임에 변화를 준데요
		{
			...
			D3DXVec3Normalize(&vDirect, &vDirect);
			pParticle->vDirect		= vDirect;
			pParticle->fSpeed		= 3.f + sc::Random::RANDOM_POS() * 3.f;
		}
...
HRESULT DxEffectSpore::CloneData ( DxEffectBase* pSrcEffect, LPDIRECT3DDEVICEQ pd3dDevice, DxTextureEffMan* pTexEff )
{
