...
void CServerManagerDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_BUTTON_AUTO_EVENT, m_AutoEventButton);
	DDX_Control(pDX, IDC_TRACING_TIME_EDIT, m_EditTracingTime);
}
...
BEGIN_MESSAGE_MAP(CServerManagerDlg, CDialog)
    ...
    //ON_BN_CLICKED(IDC_BUTTON_CRASH_TEST, &CServerManagerDlg::OnBnClickedButtonCrashTest)
    ON_BN_CLICKED(IDC_BUTTON_AUTO_EVENT, &CServerManagerDlg::OnBnClickedButtonAutoEvent)
    ON_MESSAGE(WM_UPDATE_LOG_CONSOLE, OnUpdateLogConsole)	
    ON_BN_CLICKED(IDC_BUTTON_OPEN_INSPE_DIALOG, &CServerManagerDlg::OnBnClickedButtonOpenInspeDialog)
END_MESSAGE_MAP()
...
BOOL CServerManagerDlg::OnInitDialog()
{
    ...
    sc::LogMan::getInstance()->attachEditControl(GetDlgItem(IDC_EDIT_CONSOLE)->m_hWnd, GetDlgItem(IDC_EDIT_CONSOLE)->m_hWnd);

	// Create cfg module
	CServerManagerCfg::GetInstance()->Load();

    std::string CountryMsg;

//#if defined(KR_PARAM) || defined ( GS_PARAM ) || defined ( ID_PARAM ) || defined(TEST) || defined(_RELEASED)
	if (m_ServiceProvider == SP_OFFICE_TEST || 
        m_ServiceProvider == SP_KOREA || 
        m_ServiceProvider == SP_KOREA_TEST || 
        m_ServiceProvider == SP_GS || 
        m_ServiceProvider == SP_INDONESIA)
	{
		int nRetCode;
		nRetCode = COdbcManager::GetInstance()->OpenSerManDB(
			CServerManagerCfg::GetInstance()->m_strOdbcName,														
		...
		{
            CountryMsg = sc::string::format("Country.luc country code %1% SMS DB Open Failed", m_ServiceProvider);
			AfxMessageBox(CountryMsg.c_str(), MB_ICONEXCLAMATION|MB_OK);
			PostMessage(WM_CLOSE, 0, 0);
			// Todo : program quit
		}
        else
        {
            CountryMsg = sc::string::format("Country.luc country code %1% SMS DB Open Success", m_ServiceProvider);
            AfxMessageBox(CountryMsg.c_str(), MB_OK);
        }
//#endif
	}
    else
    {
        CountryMsg = sc::string::format("Country.luc country code %1% This country not use SMS", m_ServiceProvider);
        AfxMessageBox(CountryMsg.c_str(), MB_OK);
    }
    ...
    CString IpAddress = CServerManagerCfg::GetInstance()->GetSessionIP();
    if (IpAddress.IsEmpty())
        IpAddress = "172.16.2.87";
	m_IPAddress.SetWindowText(IpAddress);
...
//	GetDlgItem( IDC_BTN_SETTING )->EnableWindow(FALSE);
	...
	{
		CWnd* pWnd = GetDlgItem(IDC_SVR_CHECK1+i);
        if (pWnd)
...
// 모든 국가에서 Tracing 사용 가능하도록 수정
////#if defined(_RELEASED) || defined(TW_PARAM) || defined(TH_PARAM) || defined(HK_PARAM) // ***Tracing Log print
//	if (m_ServiceProvider == SP_OFFICE_TEST || 
//        m_ServiceProvider == SP_TAIWAN || 
//        m_ServiceProvider == SP_HONGKONG || 
//        m_ServiceProvider == SP_THAILAND ||
//		m_ServiceProvider == SP_KOREA )
//	{
//		GetDlgItem( IDC_TRACING_BUTTON )->ShowWindow(TRUE);
//	}
...
void CServerManagerDlg::InitListHead()
{
	...
	Col.cx			= 85;
	Col.fmt			= LVCFMT_LEFT;	
	Col.pszText		= _T("Left(min)");
	Col.iSubItem	= 0;
	Col.iImage		= 0;
	m_TracingList.InsertColumn(4, &Col);
}
...
void CServerManagerDlg::OnBnClickedSendType2()
{
	...
	if (servSize)
	{
		for (size_t i=0; i< servSize; i++)
		{
			...
			{
				if (gsid->gsi.nServerGroup-1 >= 0 && gsid->gsi.nServerGroup-1 < MAX_SERVER_GROUP)
                {
					CWnd* pWnd = GetDlgItem(IDC_SVR_CHECK1+gsid->gsi.nServerGroup-1);
                    if (pWnd)
...
//		{
//			gsid = m_vecServerList[i];
//			if (gsid->gsi.nServerType == SERVER_AGENT)
//			{
...
/*
void CServerManagerDlg::OnBnClickedButtonCrashTest()
{
    // TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
    int* ptr = 0;
    *ptr = 0;
...
void CServerManagerDlg::SetAutoEventButton(bool Running)
{
    if (Running)
    {
        m_bAutoEventRun = true;
        m_AutoEventButton.SetWindowText("Reload / Event.lua");
    }
    else
    {
        m_bAutoEventRun = false;
        m_AutoEventButton.SetWindowText("Not running / Event.lua");
    }
}
void CServerManagerDlg::OnBnClickedButtonAutoEvent()
{
    // TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
    if (m_bAutoEventRun)
    {
...
LRESULT CServerManagerDlg::WindowProc(UINT message, WPARAM wParam, LPARAM lParam)
{
    // TODO: 여기에 특수화된 코드를 추가 및/또는 기본 클래스를 호출합니다.
    return CDialog::WindowProc(message, wParam, lParam);
}

LRESULT CServerManagerDlg::OnUpdateLogConsole(WPARAM wParam, LPARAM lParam)
{
