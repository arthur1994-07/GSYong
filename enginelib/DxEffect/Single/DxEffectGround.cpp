...
// Decal 로 저장된 것을 Ground 로 변형 시켜준다.
HRESULT GROUND_PROPERTY::LoadFile_DecalProp ( sc::BaseStream &SFile, LPDIRECT3DDEVICEQ pd3dDevice, BOOL bBackUpCache )
{
	...
	{
		m_Property.m_fSizeStart		= sDecalProp.m_fSizeStart * sDecalProp.m_vArea.x;	// 만약 x 와 z 가 다른게 있다면 망함.
	...
	{
		m_Property.m_fSizeStart		= sDecalProp.m_vArea.x;	// 만약 x 와 z 가 다른게 있다면 망함.
