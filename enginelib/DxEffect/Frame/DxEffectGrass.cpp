	...
	{
		{
			...
			vPos.x = pBass->vPos.x + fLength*((sc::Random::RANDOM_POS() + sc::Random::RANDOM_POS())-1.f);	// 새로운 위치
			vPos.y = 0.f;
...
HRESULT DxEffectGrass::CloneData ( DxEffectBase* pSrcEffect, LPDIRECT3DDEVICEQ pd3dDevice, DxTextureEffMan* pTexEff )
{
	...
	// [shhan][2013.07.24] 새맵은 DxEffectGrass 사용되지 않으므로, 아래 Code 는 필요없다.
	//
